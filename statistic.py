...
def run_statistic(H1, H2, H3, number_of_persons):
    ...
    if H2:
        ...
        # TODO this is the statistic for unrewared decisions
        # change it to a linear mixed effect model
        ...
        # no random intercept because dbs-off is equal for all subjects
        for session in [1, 2, 3]:
            ...
            # save results
            with open(
                f"statistic/simulation_data_difference_dbs_on_{number_of_persons}_session_{session}.txt",
                "w",
            ) as fh:
